...
public class MapFunctions
{
    ...
    public static int[,] RandomWalkTop(int[,] map, float seed)
    {
        ...
        // Remove 4 groups of columns with a length of 2
        for (int i = 0; i < 8; i++)
        {
            // Find a random start index for the columns to remove
            int startCol = rand.Next(0, map.GetUpperBound(0) - 1);

            // Ensure we don't go out of bounds
            if (startCol + 1 > map.GetUpperBound(0))
            {
                startCol = map.GetUpperBound(0) - 1;
            }

            // Set the columns to 0
            for (int x = startCol; x <= startCol + 1; x++)
            {
                for (int y = 0; y <= map.GetUpperBound(1); y++)
                {
                    map[x, y] = 0;
    ...
    public static int[,] ResizeMap(int[,] originalMap, int newWidth, int newHeight)
    {
        int[,] resizedMap = new int[newWidth, newHeight];

        for (int y = 0; y < newHeight; y++)
        {
            for (int x = 0; x < newWidth; x++)
            {
                if (x < originalMap.GetLength(0) && y < originalMap.GetLength(1))
                {
                    resizedMap[x, y] = originalMap[x, y];
                }
                else
                {
                    resizedMap[x, y] = 0; // Default value or whatever is suitable for your use case
                }
            }
        }
